...
Завершено 10.10.2023

## Introduction

В данном проекте предстоит закрепить на практике знания SQL.
Нужно будет создать базу данных со знаниями о клиентах розничных сетей, а также написать представления и процедуры, необходимые для создания персональных предложений.
...
Скрипт *part1.sql*, создающий базу данных и таблицы, описанные выше в разделе [входные данные](#входные-данные).

Также есть процедуры, позволяющие импортировать и экспортировать данные для каждой таблицы из файлов/в файлы с расширением *.csv* и *.tsv*. \
...
Для добавления данных в таблицы использовались *csv*

## Part 2. Создание представлений

Скрипт *part2.sql* - написаны представления, описанные выше в разделе [выходные данные](#выходные-данные).
Также в скрипте тестовые запросы для каждого представления.
...
Скрипт *part4.sql*, в который внесена описанная далее функция.

### Функция, определяющую предложения, ориентированные на рост среднего чека
...
Скрипт *part5.sql*, в который внесена описанная далее функция.

### Функция, определяющую предложения, ориентированные на рост частоты визитов
Параметры функции:
...
Скрипт *part6.sql*, в внесена описанная далее функция.
